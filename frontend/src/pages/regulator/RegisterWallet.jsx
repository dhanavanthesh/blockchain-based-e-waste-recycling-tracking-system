...
const RegulatorRegisterWallet = () => {
  ...
  const checkIfRegistered = React.useCallback(async () => {
    ...
    try {
      const hasRegulatorRole = await hasRole(account, 4);
      setIsRegistered(hasRegulatorRole);

      if (hasRegulatorRole) {
        setActiveStep(3);
        setSuccess('Your wallet is already registered as Regulator on the blockchain!');
      }
    } catch (error) {
  ...
  const handleRegisterOnBlockchain = async () => {
    ...
    try {
      ...
      if (hasRegulatorRole) {
        ...
        setActiveStep(2);
        setSuccess('Wallet already registered as Regulator! Linking to account...');
      ...
      if (registered) {
        console.log('Wallet registered with another role. Adding Regulator role...');
        setSuccess('Adding Regulator role to your wallet...');
      } else {
        console.log('Registering wallet for the first time as Regulator...');
      }

      const result = await registerUser('regulator');
      console.log('Registration successful:', result);

      setIsRegistered(true);
      setActiveStep(2);
      setSuccess('Successfully registered as Regulator! Now linking wallet to your account...');
  ...
  const handleLinkWallet = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        setActiveStep(3);
        setSuccess('Wallet successfully linked! You can now view and track devices.');
      ...
      <Container maxWidth="md">
        <Paper elevation={2} sx={{ p: 4 }}>
          <Box textAlign="center" mb={4}>
            <WalletIcon sx={{ fontSize: 64, color: 'secondary.main', mb: 2 }} />
            <Typography variant="h4" fontWeight="bold" gutterBottom>
              Register Your Wallet
            </Typography>
            <Typography variant="body2" color="text.secondary">
              Register as a Regulator to view device history and track ownership
            </Typography>
          </Box>
          ...
          {isConnected && !isRegistered && activeStep === 1 && (
            <Box textAlign="center" py={4}>
              ...
              <Typography variant="body2" color="text.secondary" mb={3}>
                Register your wallet address on the blockchain as a Regulator
              </Typography>
              <Button
                variant="contained"
                size="large"
...
export default RegulatorRegisterWallet;
